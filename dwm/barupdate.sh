...
#for pullin the current use of the harddrive in / and /home 7th
...
# better ways to do this. It's my approach.
# Variables are decremented slightly to stop an initial pull which
# That may happen too quickly and cause bad output (maybe)


while true; do
	#for the date portion of the string every minute
	if (( date_count==60)); then
		"/home/aaron/.scripts/dwm/update_date.sh"
		date_count=1
	else
		(( date_count++))
	fi
...
#		"/home/aaron/.scripts/dwm/update_battery.sh"
#		battery_count=1
#	else
#		((battery_count++))
#	fi
	...
	if ((internet_status_count==5)); then
		"/home/aaron/.scripts/dwm/updated_internet_status.sh"
...
#		"/home/aaron/.scripts/dwm/update_brightness.sh"
#		bright_count=1
#	else
#		((bright_count++))
#	fi
	...
	if ((weather_count==1800)); then
		"/home/aaron/.scripts/dwm/update_weather.sh"
		weather_count=1
	else
		((weather_count++))
	fi

	#for the disk use in the / and home parittions, runs every 3 minutes
	if ((disk_use_count==180)); then
		"/home/aaron/.scripts/dwm/update_disk_use.sh"
		disk_use_count=1
	else
		((disk_use_count++))
	fi
